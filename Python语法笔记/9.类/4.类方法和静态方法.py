...
#在上面，我们使用了 classmethod 装饰方法 class_foo，它就变成了一个类方法，
#class_foo 的参数是 cls，代表类本身，当我们使用 A.class_foo() 时，
#cls 就会接收 A 作为参数。另外，被 classmethod 装饰的方法由于持有 cls 参数
#因此我们可以在方法里面调用类的属性、方法，比如 cls.bar。


#静态方法

#在类中往往有一些方法跟类有关系，但是又不会改变类和实例状态的方法，
#这种方法是静态方法，我们使用 staticmethod 来装饰，比如下面的例子：
...
#我们当然可以把它写到类外面，但这是不推荐的，因为这不利于代码的组织和命名空间的整洁。
